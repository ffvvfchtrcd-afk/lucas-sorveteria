...
export default function SorvetesPage() {
  ...
  const criticos = getItensCriticos({ acai: [], sorvetes: itens, materias_primas: [], personalizados: [] })
  const baixos = getItensBaixo({ acai: [], sorvetes: itens, materias_primas: [], personalizados: [] })

  return (
    <div className="space-y-6" key={version}>
      <div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">🟠 Estoque — Sorvetes</h1>
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Acompanhe e gerencie sorvetes, picolés e coberturas. Itens em <span className="text-yellow-500">amarelo</span> precisam de reposição, em <span className="text-red-500">vermelho</span> estão críticos.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatusCard titulo="Total de Itens" valor={itens.length} descricao="itens cadastrados nesta categoria" cor="#E07B39" />
      ...
      <div>
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Lista de Itens — Sorvetes</h2>
