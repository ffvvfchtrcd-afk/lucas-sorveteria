...
export default function AcaiPage() {
  ...
  const criticos = getItensCriticos({ acai: itens, sorvetes: [], materias_primas: [] })
  const baixos = getItensBaixo({ acai: itens, sorvetes: [], materias_primas: [] })

  return (
    <div className="space-y-6" key={version}>
      <div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">🟣 Estoque — Açaí</h1>
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Gerenciamento de insumos para açaí</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatusCard titulo="Total de Itens" valor={itens.length} descricao="itens cadastrados" cor="#7B2D8E" />
